				...
				size_t        burstSize)
{
  ...
  if (m_emem.phy_base <= address
      && address + burstSize - m_emem.phy_base < m_emem.map_size)
    return e_write (&m_emem, 0, 0, address - m_emem.phy_base, buf, burstSize);
				 ...
				 size_t    burstSize)
{
  ...
  if (m_emem.phy_base <= address
      && address + burstSize - m_emem.phy_base < m_emem.map_size)
    return e_read (&m_emem, 0, 0, address - m_emem.phy_base, buf, burstSize);
