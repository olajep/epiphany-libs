				     ...
				     unsigned& offset) const
{
  ...
  if (abs_row < m_platform.row || m_platform.row + m_platform.cols <= abs_row)
    return false;

  if (abs_col < m_platform.col || m_platform.col + m_platform.cols <= abs_col)
    return false;

  row = abs_row - m_platform.row;
  col = abs_col - m_platform.col;
...
{
  return (m_emem.phy_base <= addr && addr - m_emem.phy_base < m_emem.map_size);
}	// isExternalMem ();
...
TargetControlHardware::initPlatform (unsigned int verbose)
{
  ...
  if (e_get_platform_info (&m_platform) != E_OK)
    return 1;

  /* TODO: e-hal only supports access to first memory segment */
  //e_alloc(&m_emem, 0, m_platform.emem[0].size);
  e_alloc(&m_emem, 0, 0x02000000);


  if (e_open (&m_dev, 0, 0, m_platform.rows, m_platform.cols) != E_OK)
    return 1;
				...
				size_t        burstSize)
{
  ...
  if (m_emem.ephy_base <= address
      && address + burstSize - m_emem.ephy_base < m_emem.emap_size)
    return e_write (&m_emem, 0, 0, address - m_emem.ephy_base, buf, burstSize);

  if (!addrToCoords(address, row, col, offset))
    return 0;

  size = e_write (&m_dev, row, col, offset, buf, burstSize);
				 ...
				 size_t    burstSize)
{
  ...
  if (m_emem.ephy_base <= address
      && address + burstSize - m_emem.ephy_base < m_emem.emap_size)
    return e_read (&m_emem, 0, 0, address - m_emem.ephy_base, buf, burstSize);

  if (!addrToCoords(address, row, col, offset))
    return 0;

  size = e_read (&m_dev, row, col, offset, buf, burstSize);
