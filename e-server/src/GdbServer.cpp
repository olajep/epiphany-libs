...
GdbServer::rspDetach ()
{
  ...
  // FIXME, close only when gdb does.
  if (!mExtendedMode)
    rsp->rspClose ();
  mCurrentThread = NULL;
}	// rspDetach ()
...
{
  // We don't support this packet.  RSP specification is to return an
  // empty packet.
  if (si->debugTranDetail ())
...
GdbServer::rspSetThread ()
{
  ...
  GdbTid tid = GdbTid::from_string (&pkt->data[2]);
...
{
  GdbTid tid = GdbTid::from_string (&pkt->data[1]);
...
GdbServer::rspVCont ()
{
  ...
  for (size_t i = 1; i < elements.size (); i++)
      ...
      else if (2 == tokens.size ())
	{
	  ...
	  tid_action.tid = GdbTid::from_string (tokens[1].c_str ());
	  tid_action.kind = extractVContAction (tokens[0]);
	  threadActions.push_back (tid_action);
	}
      else
...
align_down (uint32_t ptr, size_t align)
{
  return ptr & -align;
}
			    ...
			    uint32_t mem_addr, uint8_t* mem_buf, size_t len)
{
  const unsigned int bp_size = 2;
  uint32_t mem_end = mem_addr + len;
  uint32_t check_addr = align_down (mem_addr, bp_size);
			      ...
			      uint32_t mem_addr, uint8_t* mem_buf, size_t len)
{
  const unsigned int bp_size = 2;
  uint32_t mem_end = mem_addr + len;
  uint32_t bp_addr = align_down (mem_addr, bp_size);
